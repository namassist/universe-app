/**
 * The roster grid's two readings: a month, and one day of it.
 *
 * The month view was always there. What is worth pinning is the day view and
 * the tally beside it, because both are easy to get subtly wrong in ways no
 * screen would reveal: a summary folded out of the page's cells would be
 * correct on page one and wrong on page two, and a tally that ignored the
...
type Cell = { nik: string; date: string; code: "D" | "N" | "OFF" | "CR" };
...
describe("the whole month", () => {
  test("keeps every day as a column and tallies the month", async () => {
    ...
    // Every cell of both days, which is what a month-wide tally means.
    expect(counts(g)).toEqual({ D: 3, N: 3, OFF: 2, CR: 1 });
  });
});

describe("one day of it", () => {
  test("narrows the grid to that day's column", async () => {
    const g = await grid({ date: D1 });
  ...
  test("counts that day and not the one beside it", async () => {
    expect(counts(await grid({ date: D1 }))).toEqual({
      D: 3,
      N: 2,
      OFF: 1,
      CR: 1,
    });
    expect(counts(await grid({ date: D2 }))).toEqual({ N: 1, OFF: 1 });
  });

  test("rows are the people rostered that day, nobody else", async () => {
    const g = await grid({ date: D2 });
  ...
  test("the tally does not move when the page does", async () => {
    const first = await grid({ date: D1, pageSize: "2", page: "1" });
    const second = await grid({ date: D1, pageSize: "2", page: "2" });
  ...
  test("a search narrows the tally with the table", async () => {
    const g = await grid({ date: D1, q: "990500004" });
  ...
  test("a day the document has nothing for is empty, not an error", async () => {
    const g = await grid({ date: "1999-07-28" });
