# employee-data Specification

## Purpose

The employee records the rest of the UNIVERSE platform hangs off: NIK as the
unique business key, the catalogued attributes stored as references rather than
free text, the SIMPER and medical values held as the value currently in force,
the qualification codes that decide unit eligibility, employment status as a
two-valued fact distinct from dated absence, the referential protection that
keeps an account from losing the employee its NIK resolves to, photos stored on
the filesystem rather than in the database, and server-side search and filtering
for the screens that read and write them.
