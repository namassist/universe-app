...
The fleet's units as persisted records: the fields a unit carries, the
catalogue references that replace the free text a unit's class, type, model,
brand, qualification code, and department used to be, the departure schedule
that makes a bus a bus, and the rule that every screen offering units for
selection derives them from the registry.
