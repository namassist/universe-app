# roster-revision Specification

## Purpose

Changing a roster after it is in force: the submission that carries many
entries, the per-entry decision that leaves the rest of its submission
untouched, the from-code captured at submission that makes a stale approval
detectable, the transaction that writes an approved code onto the roster, the
freezing that follows a document into the archive, and the scope that decides
whose days an entry may name.
