# allocation-schedule Specification

## Purpose

The morning allocation schedule as runtime-managed timeline stages: the stage
record and who may change it, the action vocabulary shared between the database,
the API, and the client, the scheduler that fires each active stage once a day
and only from one process, and the handler extension point that lets the
allocation work be attached later without touching any of it.
